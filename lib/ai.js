...
// 目標管理コマンド
async function handleGoalCommand(text) {
  const { buildDailyTaskMessage, buildWeeklyMessage, buildMonthlyMessage, buildRoadmapMessage } = await import("../scripts/daily-task.mjs");
  if (/今日のタスク|今日は何/.test(text)) return buildDailyTaskMessage();
  if (/今週のタスク|今週は何/.test(text)) return buildWeeklyMessage();
  if (/今月の目標|今月は何/.test(text)) return buildMonthlyMessage();
  if (/ロードマップ/.test(text)) return buildRoadmapMessage();
  return null;
}

function isGoalCommand(text) {
  return /今日のタスク|今週のタスク|今月の目標|ロードマップ|今日は何|今週は何|今月は何/.test(text);
}
...
export async function handleInstruction(text) {
  ...
  if (isGoalCommand(text)) {
    const reply = await handleGoalCommand(text);
